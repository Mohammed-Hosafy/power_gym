...
class SignUpScreenState extends State<SignUpScreen>
  ...
  Future<void> _signUp() async {
    ...
    if (height == null || weight == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Height and Weight must be numbers")),
      );
      return;
    }
    ...
    String userId = usernameCtrl.text;
    await FirestoreSetup.createUser(
      userId,
    ...
    await FirestoreSetup.addSchedule(userId, 'schedule1', 'workout1', 'Monday');
    await FirestoreSetup.addCardio(userId, 'cardio1', 30);
    await FirestoreSetup.addDiet(userId, 'diet1', 2500);

    // تمرير الـ username عند الانتقال للصفحة الرئيسية
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
          builder: (_) => HomePage(username: usernameCtrl.text.trim())),
    );
  }
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 12),
                      _buildTextField(
                          ctrl: passwordCtrl, hint: "Password", obscure: true)
                          .animate()
                          .fade(duration: 500.ms)
                          .slideX(begin: -0.3, end: 0),
                      const SizedBox(height: 12),
                      _buildTextField(
                          ctrl: confirmCtrl, hint: "Confirm Password", obscure: true)
                          .animate()
                          .fade(duration: 500.ms)
                          .slideX(begin: -0.3, end: 0),
                      const SizedBox(height: 25),
